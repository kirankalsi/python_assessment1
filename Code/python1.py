...
def two(input):
    ...
    split = lower_case_input.split('bert') 
    if len(split) == 3:
        return split[1]
    return ""
...
def nine(inputString, char):
    ...
    inputString = [letter for letter in inputString]
...
def ten(string, int, char):
    ...
    search = lower_case_string.index(char)
    if search == int - 1:
        return True
    return False